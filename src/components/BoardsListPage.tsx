...
interface Props {
  ...
  boardAccessMap: Record<string, { adminIds: string[]; memberIds: string[] }>;
  ...
  onUpdateBoardAccess: (boardId: string, access: { adminIds: string[]; memberIds: string[] }) => void;
...
export default function BoardsListPage({
  ...
  const findBoardById = (boardId: string) =>
    workspaces.flatMap((w) => w.boards).find((b) => b.id === boardId) || null;

  const accessBoard = accessBoardId ? findBoardById(accessBoardId) : null;
  const access = accessBoard ? (boardAccessMap[accessBoard.id] || { adminIds: [], memberIds: [] }) : null;

  const getMemberById = (id: string) => ALL_SYSTEM_MEMBERS.find((m) => m.id === id);

  const openAccessModal = (boardId: string) => {
    setAccessBoardId(boardId);
    setNewAdminId('');
    setNewMemberId('');
    const current = boardAccessMap[boardId] || { adminIds: [], memberIds: [] };
    setActingAdminId(current.adminIds[0] || '');
  };
  ...
  const handleAddAdminToBoard = () => {
    if (!accessBoard || !access) return;
    const id = newAdminId.trim();
    const exists = getMemberById(id);
    if (!id || !exists) return;
    if (access.adminIds.includes(id)) return;

    onUpdateBoardAccess(accessBoard.id, {
      ...access,
      adminIds: [...access.adminIds, id],
    });
    if (!actingAdminId) setActingAdminId(id);
    setNewAdminId('');
  };

  const handleRemoveAdminFromBoard = (adminId: string) => {
    if (!accessBoard || !access) return;
    const nextAdmins = access.adminIds.filter((id) => id !== adminId);
    onUpdateBoardAccess(accessBoard.id, {
      ...access,
      adminIds: nextAdmins,
    });
    if (actingAdminId === adminId) setActingAdminId(nextAdmins[0] || '');
  };

  const handleAddMemberToBoard = () => {
    if (!accessBoard || !access || !actingAdminId) return;
    const id = newMemberId.trim();
    const exists = getMemberById(id);
    if (!id || !exists) return;
    if (access.memberIds.includes(id)) return;

    onUpdateBoardAccess(accessBoard.id, {
      ...access,
      memberIds: [...access.memberIds, id],
    });
    setNewMemberId('');
  };

  const handleRemoveMemberFromBoard = (memberId: string) => {
    if (!accessBoard || !access) return;
    onUpdateBoardAccess(accessBoard.id, {
      ...access,
      memberIds: access.memberIds.filter((id) => id !== memberId),
    });
  };
              ...
              <div className="boards-grid">
                {openWorkspace.boards.filter((b) => b.starred).map((board) => (
                  <BoardTile
                    ...
                    onManageAccess={() => openAccessModal(board.id)}
    ...
    <div className="boards-page">
      ...
      <header className="boards-page-header">
        <div className="boards-page-header-inner">
          ...
          <button
            className="header-gantt-btn"
            onClick={onOpenGantt}
            title="Гант диаграм"
          >
            <span>📊</span>
            <span>Гант диаграм</span>
          </button>
          <button
      ...
      {accessBoard && access && (
        <CreateModal title={`Эрхийн удирдлага - ${accessBoard.title}`} onClose={closeAccessModal}>
          <div className="create-board-body permissions-body">
            <div className="permissions-header-note">
              Super Admin: Энгийн админы эрх олгоно. Admin: тухайн самбар дотор гишүүн томилно.
            </div>

            <div className="permissions-section">
              <div className="permissions-section-title"><Shield size={14} /> Super Admin - Board Admin нэмэх</div>
              <div className="permissions-row">
                <input
                  className="create-board-input"
                  placeholder="Админы ID (жиш: 1002)"
                  value={newAdminId}
                  onChange={(e) => setNewAdminId(e.target.value)}
                />
                <button className="btn-secondary" onClick={handleAddAdminToBoard}>Нэмэх</button>
              </div>
              <div className="permissions-list">
                {access.adminIds.length === 0 ? (
                  <span className="permissions-empty">Энэ самбарт админ оноогоогүй байна.</span>
                ) : (
                  access.adminIds.map((id) => {
                    const m = getMemberById(id);
                    return (
                      <div key={id} className="permissions-item">
                        <span>{m ? `${m.name} (${id})` : id}</span>
                        <button className="ws-action-btn danger" onClick={() => handleRemoveAdminFromBoard(id)}>
                          <Trash2 size={12} />
                        </button>
                      </div>
                    );
            ...
            <div className="permissions-section">
              <div className="permissions-section-title"><UserPlus size={14} /> Admin - Board гишүүн томилох</div>
              <div className="permissions-row">
                <select
                  className="create-board-input permissions-select"
                  value={actingAdminId}
                ...
                >
                  ...
                  {access.adminIds.map((id) => {
                    const m = getMemberById(id);
                    return <option key={id} value={id}>{m ? `${m.name} (${id})` : id}</option>;
                  })}
                </select>
              </div>
              ...
              <div className="permissions-row">
                ...
                />
                <button className="btn-secondary" onClick={handleAddMemberToBoard} disabled={!actingAdminId}>Томилох</button>
              </div>

              <div className="permissions-list">
                {access.memberIds.length === 0 ? (
                  <span className="permissions-empty">Энэ самбарт гишүүн оноогоогүй байна.</span>
                ) : (
                  access.memberIds.map((id) => {
                    const m = getMemberById(id);
                    return (
                      <div key={id} className="permissions-item">
                        <span>{m ? `${m.name} (${id})` : id}</span>
                        <button className="ws-action-btn danger" onClick={() => handleRemoveMemberFromBoard(id)}>
                          <Trash2 size={12} />
                        </button>
                      </div>
                    );
